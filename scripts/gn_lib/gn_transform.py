...
def ecef2eci(sp3_in):
    ...
    cos_theta = _np.cos(theta)[:,_np.newaxis]
    sin_theta = _np.sin(theta)[:,_np.newaxis]

    x = sp3_in.loc(axis=1)[:,'X'].values
    y = sp3_in.loc(axis=1)[:,'Y'].values

    x_eci = x*cos_theta - y*sin_theta
    y_eci = x*sin_theta + y*cos_theta

    sp3_new.loc(axis=1)[:,'X'] = x_eci
    sp3_new.loc(axis=1)[:,'Y'] = y_eci
    return sp3_new
