	...
	KFState&		kfState)
{
	...
	for (auto& [kfKey, index] : kfState.kfIndexMap)
	{
		...
		if (0)
		{
			...
			{
				kfState.getKFValue(KFKey{.type = KF::SAT_POS,		.Sat = SatSys(E_Sys::GPS, 1), .num = i}, rSat[i]);
				kfState.getKFValue(KFKey{.type = KF::SAT_POS_RATE,	.Sat = SatSys(E_Sys::GPS, 1), .num = i}, vSat[i]);
			}
			
			std::cout << std::endl << "Mismatches are:\n";
			std::cout << std::endl << "rsat " << (rSat - newStates.topRows(3)).transpose();
			std::cout << std::endl << "vsat " << (vSat - newStates.bottomRows(3)).transpose();
		}
	}
}
