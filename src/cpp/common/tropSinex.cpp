	...
	bool isSmoothed)		///< if solution is smoothed (RTS or fixed-lag)
{
	...
	if (acsConfig.netwOpts.filter_mode == +E_FilterMode::LSQ)		tropEstMethod = "Least Square";
	if (acsConfig.netwOpts.filter_mode == +E_FilterMode::KALMAN)	tropEstMethod = "Filter";
	if (isSmoothed)													tropEstMethod = "Smoother"; //overrides "Filter"
