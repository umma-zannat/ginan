	...
	GTime	time)
{
	...
	{
		cullSSRMap(time, satNav.ssr.ssrCodeBias_map);
		cullSSRMap(time, satNav.ssr.ssrPhasBias_map);
		cullSSRMap(time, satNav.ssr.ssrClk_map);
		cullSSRMap(time, satNav.ssr.ssrEph_map);
		cullSSRMap(time, satNav.ssr.ssrHRClk_map);
		cullSSRMap(time, satNav.ssr.ssrUra_map);
	}
}
	...
	int&			iodEph)
{
	...
	auto ephIt = ssr.ssrEph_map.lower_bound(time);
	if (ephIt == ssr.ssrEph_map.end())	
	{
		return false;
	}
...
//         tracepdeex(3,"age of ssr error: %s t=%.0f > %.0f\n",time.to_string(0).c_str(), tEph, ssrEph.udi * acsConfig.validity_interval_factor);
	...
	{
//         trace(3,"invalid ssr correction: %s deph=%.1f\n", time.to_string(0).c_str(), norm(deph,3));
		return false;
	}
...
{
	//get 'price is right' closest ssr components to ephemeris time.
	auto clkIt = ssr.ssrClk_map.lower_bound(time);	
	if (clkIt == ssr.ssrClk_map.end())	
	{
		return false;
	}
...
//         trace(2,"age of ssr error: %s sat=%2d t=%.0f %.0f\n", time.to_string(0).c_str(), obs.Sat,t1,t2);
	...
	auto hrcIt = ssr.ssrHRClk_map.lower_bound(time);
...
//     tracepdeex(4,trace, __FUNCTION__ ": time=%s sat=%2d\n",time.to_string(3).c_str(),obs.Sat);
	...
	Vector3d	dPos;
	double		dClk;
	if	(  !ssrPosDelta(time, ssr, dPos, iodPos, iodEph)
		|| !ssrClkDelta(time, ssr, dClk, iodClk))
	{
		return false;
	}
...
// 		std::cout << "Bad SSR Delta function" << std::endl;
	...
	auto uraIt = ssr.ssrUra_map.lower_bound(time);
